// TODO:
// how can we do max subscriptions across all connections?
//   for each mqserver which connects, on (un)subscribe_requested (either from
//   ac or mqs) check if it is going to sub/unsub and maintain count
// max num of 'active' messages?
//   has to be done in qlobber-fsq
// config should have some defaults for max values like frame maxSize, max header size etc? or at least list everything which can be set - inc where have done tests to do using events/hooks etc
