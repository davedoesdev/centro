// TODO:
// why do we replace presence join content?
//   - was to stop someone else saying someone else has joined
//   - but can't we do that using access control?
// if error does it clean up (unsub from fsq)?
// unsub - check internal maps and matchers are cleaned up
// 100% coverage
// add test for fastest-writable, stream timeout and max data transferred
//   (max publish), throttle?
// cli
            ...
            async.mapSeries(transports, function (transport, cb)
            {
                function run_transport(config, authz, transport, keystore)
                {
                    ...
                    }, function (handshakes, stream, destroy, onclose)
                    {
                        ...
                        dstroy.destroy = destroy; // for testing
