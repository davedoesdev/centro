...
function CentroServer(config)
{
    ...
    this.fsq = new QlobberFSQ(config);
    ...
    this.fsq.on('start', function ()
    {
        config.authorize(config, function (err, authz)
        {
            ...
            async.mapSeries(config.transports || [config.transport], function (transport, cb)
            {
                    ...
                    onclose(function ()
                    {
                        for (var conn of hs_conns)
                        {
                            ...
                            if (conns !== undefined)
                            {
                                ...
                                if (conns.size === 0)
                                {
                                    ths._connections.delete(handshake.uri);
                                }
                            }
                        }
...
CentroServer.prototype.close = function (cb)
{
    ...
    async.each(this._closes, function (close, cb)
    {
        close(cb);
    }, function (err, cb)
    {
