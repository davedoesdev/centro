// TODO:
// subscribe here should be async; errors close or just ignored?
// do topic length checks in access-control
//   and max subs
//   and max data published (in a way that can be chained with app handler)
//   remember: access-control is only for client-driven stuff
//   other checks must be in json-schema
...
function CentroServer(config)
{
    ...
    function compile(required)
    {
        return ajv.compile({
            ...
            properties: {
                    ...
                    properties: {
                        publish: {
                            ...
                            properties: {
                                allow: {
                                    type: 'array',
                                    maxItems: config.max_access_control_publish_allow,
                                    items: {
                                        type: 'string'
                                    }
                                },
                                disallow: {
                                    type: 'array',
                                    maxItems: config.max_access_control_publish_disallow,
                                    items: {
                                        type: 'string'
                                    }
                                }
                        ...
                        subscribe: {
                            ...
                            properties: {
                                allow: {
                                    type: 'array',
                                    maxItems: config.max_access_control_subscribe_allow,
                                    items: {
                                        type: 'string'
                                    }
                                },
                                disallow: {
                                    type: 'array',
                                    maxItems: config.max_access_control_subscribe_disallow,
                                    items: {
                                        type: 'string'
                                    }
                                }
                        ...
                        block: {
                            type: 'array',
                            maxItems: config.max_access_control_block,
                            items: {
                                type: 'string'
                            }
                        }
                ...
                subscribe: {
                    type: 'object',
                    maxProperties: config.max_subscribe,
    ...
    function authorize(err, default_authz)
    {
        ...
        {
            function transport_authorize(obj, destroy, cb2)
            {
                ...
                function got_tokens(err, tokens)
                {
                    ...
                    {
                        if (config.max_token_size && (token.length > config.max_token_size))
                        {
            ...
            function transport_connected(handshakes, stream, destroy, onclose)
            {
                ...
                function processed_handshakes(err)
                {
                    ...
                    mqserver.on('handshake', function (hsdata, delay)
                    {
                        ...
                        {
                            return warning(new Error('unsupported version: ' + client_version));
                        }
