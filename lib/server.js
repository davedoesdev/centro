...
/*
in server_extensions:  (also, re-write tests to use them)

fastest-writable
delay message until all streams under hwm
limit data pub per conn
limit num msgs published per conn
limit num msgs received per conn?
...
mqlobber:
  ...
  - defer_to_final_handler (let all handle message. false)
  - message event
  - ** backoff event (use filter + _requested handlers to allow conn to drain? or close conn?)
                    // have config options to 
                    // close conn on backoff
                    // install filter and _requested handlers to delay things

  - drain event
  - full event
*** should we have option to close on any error event (or full event)?
  - removed event
...
tests:
  ...
  - limit data published per message - already in mqlobber-access-control (max_publish_data_length)
  ...
  - limit number of publications across all connections
        ...
        function run_transport(config, authz, transport, keystore_to_close, next)
        {
            ...
            function transport_connected(handshakes, stream, destroy, onclose)
            {
                ...
                function processed_handshakes(err)
                {
                    ...
                    {
                        warning.call(mqserver, new Error('full'));
                    });
...
{
    ext = Object.assign({ detach: function () {} }, ext(config, this));

    for (var ev in ext)
    {
        /* istanbul ignore else */
        if ((ev != 'detach') && ext.hasOwnProperty(ev))
        {
            this.on(ev, ext[ev]);
        }
    }
...
CentroServer.prototype.detach_extension = function (ext)
{
    for (var ev in ext)
    {
        /* istanbul ignore else */
