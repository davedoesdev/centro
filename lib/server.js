// embedded apps?
//   per-app anonymous mode - have transport configured with anon + no max_token_expiry or audience_uri
// http post support
// presence?
...
function CentroServer(config)
{
    ...
    function start()
    {
        config.authorize(config, function (err, authz)
        {
            ...
            {
                function run_transport(config, authz, transport)
                {
                ...
                else
                {
                    ...
                    if (transport.authz_config)
                    {
                        var aconfig = Object.assign({}, tconfig, transport.authz_config);
