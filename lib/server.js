...
// yes - should be, add test for fastest-writable, timeout and max size
// we should emit connect event before has handshaken - up to app to register
// for handshake event
// and also include destroy and onclose
// will need to rewrite test which uses 'connect'
// problem is we intercept publish_requested for presence so how do we let
// apps handle it on mqserver?
// modify mqlobber so it emits an event before *_requested
...
// test >1 connection over multiple transports
                    ...
                    }, function (handshakes, stream, destroy, onclose)
                    {
                        ...
                        if (presence.size > 0)
                        {
                            ...
                            function (server, topic, stream, options, cb)
                            {
                                ...
                                if (!server.emit('publish_requested', topic, stream, options, cb))
                                {
