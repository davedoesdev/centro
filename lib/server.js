 ...
 * @param {string} [config.realm=centro] - When authorization of a client fails, the transport is given information it can (optionally) use when rejecting the connection. This includes a HTTP Basic Authentication header specifying this realm.
...
exports.CentroServer = function CentroServer(config)
{
     ...
     *
     * @member {QlobberFSQ} fsq
     */
    this.fsq = null;

    /**
      * Operations you can perform on transport you specified when constructing {@link centro-js/lib/server.CentroServer|CentroServer}. Transports can define their own operations in addition to `close`. Indexable by position or transport name.
     *
     * @member {{close: centro-js/lib/server.closeCallback}[]} transport_ops
     */
...
exports.CentroServer.prototype._authz_error = function (err)
{
    ...
    return {
        statusCode: 401,
        authenticate: 'Basic realm="' + this._config.realm + '"',
        message: err.message
    };
};
    ...
    function next(i)
    {
        return function ()
        {
            ...
            else
            {
                ...
                h.apply(obj, args.concat(next(i + 1)));
            }
        };
    }
...
{
    ext = ext.call(this, config, pipeline);
 ...
 */
exports.closeCallback = function (err) {};
 ...
 */
exports.cancelCallback = function (err) {};

/**
 * Callback type for registering close handler.
 *
 * @callback oncloseCallback
 * @param {closeCallback} cb - Called when close completes.
 */
exports.oncloseCallback = function (cb) {};
...
/**
 * Authorization start event. Emitted when a client presents an authorization
 * token. This is the first event emitted when a client connects.
 *
 * @event CentroServer#authz_start
 * @param {cancelCallback} cancel - Call this to cancel authorization.
 * @param {oncloseCallback} onclose - Call this to register a handler when the connection is closed.
...
/**
 * Authorization end event. Emitted when a authorization token has been accepted
 * or rejected.
 *
 * @event CentroServer#authz_end
 * @param {?Object} err - `null` if token was accepted otherwise cause of rejection.
 * @param {{payload: Object, uri: string, rev: rev}[]} [handshakes] - The token's payload, issuer URI and public key revision (see {@link https://github.com/davedoesdev/authorize-jwt|authorize-jwt}). Normally you won't need this information.
 * @param {{string}[]} [tokens] - The raw JSON Web Tokens. Normally you won't need this information.
