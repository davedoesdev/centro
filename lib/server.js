...
// - pass an onclose to authorize in each transport then upwards
//   onready per transport, call that with the ops
//   - write tests for maxconnections on each transport
//     in-mem will have to count
// configure timeout on http requests (publish)?
//   - just handle request event in onready?
        ...
        function run_transport(config, authz, transport, keystore_to_close, next)
        {
            function transport_authorize(obj, destroy, onclose, cb2)
            {
                ...
                function cb(err, handshakes, tokens)
                {
                    ...
                    if (!authz_ended)
                    {
                        ths.emit('authz_end', obj, err, tokens);
                        authz_ended = true;
                    }
