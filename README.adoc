...
var config = {
    ...
    transports: [{
        ...
        server: 'in-mem',
        authorize_config: { ANONYMOUS_MODE: true } // <4>
    }]
};

var server = new centro.CentroServer(config);

server.on('ready', function () // <5>
{
    console.log('READY.');
});
----
...
<4> In-memory transport doesn't require authorization tokens.
<5> Connections may be refused until `ready` is emitted.

This is just an example -- you only need to list transports to which your
applications will connect. Before you run `server.js`, make sure you do
`grunt keys` first.
